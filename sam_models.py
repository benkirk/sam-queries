...
Full schema with association_proxy-based, ORM-aware relationships:

  - Project.users  -> active users on the project (via Account -> AccountUser)
  - User.projects  -> active projects for the user (via AccountUser -> Account -> Project)

Everything else is preserved from your original schema.
"""
...
from sqlalchemy import (
    ...
    ForeignKey, Text, BigInteger, TIMESTAMP, text, and_, or_
)
...
class User(Base):
    ...
    # We expose `projects` as an association proxy that refers to the `account.project` on each AccountUser.
    _projects_w_dups = association_proxy('accounts', 'account.project')

    # Association proxy: user.projects -> list of Project objects (active only via property)
    @property
    def all_projects(self):
        s=set()
        for p in self._projects_w_dups: s.add(p)
        return list(s)

    @property
    def active_projects(self):
        s=set()
        for p in self._projects_w_dups:
            if p.active:
                s.add(p)
        return list(s)
...
# ============================================================================
# Project Management (Project uses Account and AccountUser to expose users)
# ============================================================================
...
class Project(Base):
    ...
    # ---- account_users: join Project -> Account -> AccountUser, but filter on active AccountUser
    # We implement the filtering in the secondaryjoin expression using SQLAlchemy boolean functions.
    account_users = relationship(
        ...
        viewonly=True,
        lazy='selectin',
        collection_class=set,  # ← ensures uniqueness at the ORM level
    )

    @property
    def users(self):
        """Return a deduplicated list of active users on this project."""
        # Convert from set of AccountUser → set of Users, then back to list
    ...
    def get_allocation_by_resource(self, resource_name: str) -> Optional['Allocation']:
        ...
        return allocations_by_resource[resource_name] if resource_name in allocations_by_resource else None
...
# ============================================================================
# Contract / LoginType / Misc small tables
# ============================================================================
