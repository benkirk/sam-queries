...
GET endpoints (status retrieval, public with login):
    ...
    GET /api/v1/status/jupyterhub/live - Real-time JupyterHub stats (NEW)
    GET /api/v1/status/jupyterhub/live/users - Active users list (NEW)
    GET /api/v1/status/jupyterhub/live/breakdown - Job breakdown (NEW)
    GET /api/v1/status/jupyterhub/live/sessions - Detailed sessions (NEW)
...
# Import JupyterHub API client for real-time statistics
...
# ============================================================================
# JupyterHub Real-Time API Endpoints
# ============================================================================

# JupyterHub client singleton (lazy initialization)
_jupyterhub_clients = {}


def get_jupyterhub_client(instance='stable'):
    """
    Get or create JupyterHub client for instance.

    Args:
        instance: JupyterHub instance ('stable' or 'dev')

    Returns:
        JupyterHubClient instance
    """
    if instance not in _jupyterhub_clients:
        base_url = os.getenv('JUPYTERHUB_API_URL', 'https://jupyterhub.hpc.ucar.edu')
        cache_ttl = int(os.getenv('JUPYTERHUB_CACHE_TTL', '300'))
        _jupyterhub_clients[instance] = JupyterHubClient(
            base_url=base_url,
            instance=instance,
            cache_ttl=cache_ttl
        )
    return _jupyterhub_clients[instance]


@bp.route('/jupyterhub/live', methods=['GET'])
@login_required
def get_jupyterhub_live():
    """
    GET /api/v1/status/jupyterhub/live - Real-time JupyterHub statistics.

    Query params:
        - instance: 'stable' or 'dev' (default: 'stable')
        - no_cache: Set to 'true' to bypass cache

    Returns:
        JSON with active users, sessions, job breakdown

    Example response:
        {
            "active_users": 42,
            "active_sessions": 58,
            "casper_login_jobs": 15,
            "casper_batch_jobs": 30,
            "derecho_batch_jobs": 10,
            "broken_jobs": 3,
            "instance": "stable",
    ...
    """
    instance = request.args.get('instance', 'stable')
    if instance not in ('stable', 'dev'):
        return jsonify({'error': 'Invalid instance. Must be "stable" or "dev"'}), 400

    no_cache = request.args.get('no_cache', '').lower() in ('true', '1', 'yes')

    try:
        client = get_jupyterhub_client(instance)
        stats = client.get_statistics(use_cache=not no_cache)
        return jsonify(stats), 200

    except JupyterHubAuthError as e:
        return jsonify({'error': f'Authentication failed: {str(e)}'}), 401
    except JupyterHubConnectionError as e:
        return jsonify({'error': f'Connection failed: {str(e)}'}), 503
    except JupyterHubAPIError as e:
        return jsonify({'error': f'API error: {str(e)}'}), 500


@bp.route('/jupyterhub/live/users', methods=['GET'])
@login_required
def get_jupyterhub_live_users():
    """
    GET /api/v1/status/jupyterhub/live/users - Active usernames.
    ...
    Returns:
        JSON with list of active usernames

    Example response:
        {
            "users": ["benkirk", "user2", "user3"],
            "count": 3,
            "instance": "stable"
        }
    """
    ...
    try:
        client = get_jupyterhub_client(instance)
        users = client.get_active_users(use_cache=not no_cache)
...
@bp.route('/jupyterhub/live/breakdown', methods=['GET'])
@login_required
def get_jupyterhub_live_breakdown():
    """
    GET /api/v1/status/jupyterhub/live/breakdown - Job type breakdown.
    ...
    try:
        ...
        return jsonify({
            'breakdown': {
                'casper_login': stats['casper_login_jobs'],
                'casper_batch': stats['casper_batch_jobs'],
                'derecho_batch': stats['derecho_batch_jobs'],
                'broken': stats['broken_jobs']
            },
            'total_jobs': stats['active_sessions'],
            'instance': instance
        }), 200
...
@bp.route('/jupyterhub/live/sessions', methods=['GET'])
@login_required
def get_jupyterhub_live_sessions():
    """
    GET /api/v1/status/jupyterhub/live/sessions - Detailed session list.
    ...
    Returns:
        JSON with session details (username, server, resource, duration)
            ...
            "sessions": [
                {
                    "username": "benkirk",
                    "server_name": "default",
                    "resource_type": "cr-login",
                    "location": "776111.casper-pbs",
                    "duration_minutes": 150,
                    "idle_minutes": 5,
                    "started": "2025-12-16T10:00:00.000000Z",
                    "last_activity": "2025-12-16T12:25:00.000000Z"
                }
            ],
            "total_sessions": 1,
            "instance": "stable"
    ...
    try:
        client = get_jupyterhub_client(instance)
        sessions = client.get_detailed_sessions(use_cache=not no_cache)
