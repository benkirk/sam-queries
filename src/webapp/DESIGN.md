...
Permissions are defined as an enum in `webapp/utils/rbac.py`:
...
```python
from webapp.utils.rbac import require_permission, require_role, Permission
...
**Register in `run.py`:**
```python
from webapp.api.v1 import users as api_users
...
Modify `MyAdminIndexView.index()` in `webapp/admin/views.py`:

```python
@expose('/')
def index(self):
    from webapp.utils.rbac import has_role
...
2. Implement `LDAPAuthProvider.authenticate()` in `webapp/auth/providers.py`
