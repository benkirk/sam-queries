...
def _stub_gid_pool(monkeypatch):
    ...
    its real behavior stays covered by ``tests/unit/models/test_gid_allocation.py``. These
...
class TestTheFailurePaths:
    ...
    def test_problems_accumulate_into_one_422(self, committing, session,
        ...
        from factories import make_user
        pi = make_user(session)                       # no affiliation -> no mnemonic
        ...
        assert set(exc.value.messages) >= {
            ...
            f'PI {pi.username} has no current institution or organization in SAM',
        }
