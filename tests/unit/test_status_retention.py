...
Most of these tests exist because ``docs/plans/SCHEDULED_TASKS.md`` § 3.1 turned
``scripts/cleanup_status_data.py`` from a thing to port into a set of decisions
to make. Each decision that changed behaviour gets a test here, so the agenda
leaves evidence rather than a changelog entry:
