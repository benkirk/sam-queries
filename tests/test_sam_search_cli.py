...
def run_cli(*args, expect_success=True):
    ...
    result = subprocess.run(
        [CLI_PATH] + list(args), capture_output=True, text=True, timeout=30
    )
...
class TestProjectExactSearch:
    ...
    def test_project_with_list_users(self):
        ...
        assert "PROJECT INFORMATION" in result.stdout
        assert (
            "users for SCSG0001" in result.stdout.lower() or "benkirk" in result.stdout
        )
...
class TestProjectExpirations:
    ...
    def test_recent_expirations_with_users(self):
        ...
        assert (
            "abandoned" in result.stdout.lower() or "expiring" in result.stdout.lower()
        )
    ...
    def test_upcoming_expirations_verbose(self):
        ...
        if "days remaining" in result.stdout:
            assert (
                "PROJECT INFORMATION" in result.stdout
                or "Use --verbose" not in result.stdout
            )
...
class TestCLIBehavior:
    ...
    def test_global_flag_inactive_users(self):
        ...
        result = run_cli(
            "--inactive-users", "user", "--search", "test%", "--limit", "5"
        )
...
class TestEdgeCases:
    ...
    def test_limit_zero(self):
        ...
        result = run_cli(
            "user", "--search", "ben%", "--limit", "0", expect_success=False
        )
