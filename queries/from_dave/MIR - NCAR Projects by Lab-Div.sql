select pop.panel_name as "Type"
, pop.acronym as "Division"
, count(distinct pop.projcode) as "Projects"
, sum(hpc.gaus)/3.4 as "Cheyenne CHs"
, sum(hpc2.gaus)/3.4 as "Derecho CHs"
, sum(dav.gaus)/3.4 as "DAV CHs"
, sum(campaign.tb_yrs) as "Campaign TB-yrs"
, sum(glade.tb_yrs) as "GLADE TB-yrs"

from (select distinct pn.panel_name, o.acronym, p.projcode
from account ac
...
left join project_organization po on po.project_id = p.project_id
...
and f.facility_name in ('NCAR','ASD')
and po.project_organization_id = (select max(pox.project_organization_id) from project_organization pox
where pox.project_id = po.project_id)
) as pop
...
group by 1) as dav
on lower(dav.projcode) = lower(pop.projcode)
left outer join (select p2.projcode
    ...
    join project_directory pd on pd.directory_name = da.directory_name
	  ...
	  group by 1) as glade
		on glade.projcode = pop.projcode
left outer join (select p2.projcode
		-- , round(sum(da.file_size_total)/power(1000,3),2) as tb
		, sum(dc.charge) as tb_yrs
      from disk_activity da
    ...
    join project_directory pd on pd.directory_name = da.directory_name
	  ...
	  group by 1) as campaign
		on campaign.projcode = pop.projcode
