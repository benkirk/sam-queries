...
WHERE
    -- Filter to exclude deleted accounts and allocations and include only active projects.
    al.deleted = 0
    AND ac.deleted = 0
    AND p.active = 1
GROUP BY
    ...
    u.last_name
HAVING
    -- latest_end_date must be on or after the first day of the month 3 months prior.
    latest_end_date >= DATE_FORMAT(DATE_SUB(CURDATE(), INTERVAL 3 MONTH), '%Y-%m-01')
    AND
    -- latest_end_date must be on or before the last day of the month 3 months prior.
