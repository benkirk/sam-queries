...
class Facility(Base, TimestampMixin, ActiveFlagMixin):
    ...
    def __hash__(self):
        ...
        return (
            hash(self.facility_id) if self.facility_id is not None else hash(id(self))
        )
...
class Panel(Base, TimestampMixin, ActiveFlagMixin):
    ...
    __table_args__ = (Index("ix_panel_facility", "facility_id"),)
...
class PanelSession(Base, TimestampMixin):
    ...
    __table_args__ = (Index("ix_panel_session_panel", "panel_id"),)
...
class ProjectCode(Base):
    ...
    facility_id = Column(Integer, ForeignKey("facility.facility_id"), nullable=False)
    mnemonic_code_id = Column(
        Integer, ForeignKey("mnemonic_code.mnemonic_code_id"), nullable=False
    )
