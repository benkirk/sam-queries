...
class User(Base, TimestampMixin):
    ...
    academic_status_id = Column(
        Integer, ForeignKey("academic_status.academic_status_id")
    )
    ...
    admin_projects = relationship(
        "Project", foreign_keys="Project.project_admin_user_id", back_populates="admin"
    )
    administered_resources = relationship(
        "Resource",
    ...
    allocation_transactions = relationship(
        "AllocationTransaction",
        foreign_keys="AllocationTransaction.user_id",
        back_populates="user",
    )
    archive_charge_summaries = relationship(
        "ArchiveChargeSummary", back_populates="user"
    )
    archive_charges = relationship("ArchiveCharge", back_populates="user")
    charge_adjustments_made = relationship(
        "ChargeAdjustment",
    ...
    email_addresses = relationship(
        ...
        back_populates="user",
        lazy="selectin",
        order_by="EmailAddress.is_primary.desc()",
    )
    ...
    led_projects = relationship(
        "Project", foreign_keys="Project.project_lead_user_id", back_populates="lead"
    )
    ...
    def get_by_email(cls, session, email: str) -> Optional["User"]:
        ...
        user = (
            session.query(cls)
            .join(cls.email_addresses)
            .filter(EmailAddress.email_address == email)
    ...
    def search_by_username(
        cls, session, pattern: str, active_only: bool = True, limit: int = 50
    ) -> List["User"]:
        """
    ...
    def search_by_email(
        ...
        query = (
            session.query(cls)
            .join(cls.email_addresses)
            .filter(EmailAddress.email_address.ilike(pattern))
            .options(joinedload(cls.email_addresses))
        )
        ...
        if search_email:
            ...
            if active_only:
                email_query = email_query.filter(
                    cls.active == True, cls.locked == False
                )
    ...
    def get_active_users(cls, session, limit: Optional[int] = None) -> List["User"]:
        ...
        query = (
            session.query(cls)
            .filter(cls.active == True, cls.locked == False)
...
class EmailAddress(Base, TimestampMixin):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, EmailAddress):
            return False
        return (
            self.email_address_id is not None
            and self.email_address_id == other.email_address_id
        )

    def __hash__(self):
        """Hash based on email_address_id for set/dict operations."""
        return (
            hash(self.email_address_id)
            if self.email_address_id is not None
            else hash(id(self))
        )
    ...
    def __repr__(self):
        return (
            f"<EmailAddress(email='{self.email_address}', primary={self.is_primary})>"
        )
...
class Phone(Base, TimestampMixin):
    ...
    __table_args__ = (Index("ix_phone_user", "user_id"),)

    ext_phone_id = Column(Integer, primary_key=True, autoincrement=True)
    ext_phone_type_id = Column(
        Integer, ForeignKey("phone_type.ext_phone_type_id"), nullable=False
    )
    user_id = Column(Integer, ForeignKey("users.user_id"), nullable=False)
...
class UserResourceHome(Base, TimestampMixin):
    ...
    def __repr__(self):
        return (
            f"<UserResourceHome(user_id={self.user_id}, dir='{self.home_directory}')>"
        )
...
class UserResourceShell(Base, TimestampMixin):
    ...
    user_id = Column(Integer, ForeignKey("users.user_id"), nullable=False)
    resource_shell_id = Column(
        Integer, ForeignKey("resource_shell.resource_shell_id"), nullable=False
    )
