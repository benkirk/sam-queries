...
class DavActivity(Base):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, DavActivity):
            return False
        return (
            self.dav_activity_id is not None
            and self.dav_activity_id == other.dav_activity_id
        )

    def __hash__(self):
        """Hash based on dav_activity_id for set/dict operations."""
        return (
            hash(self.dav_activity_id)
            if self.dav_activity_id is not None
            else hash(id(self))
        )
...
class DavCharge(Base):
    ...
    dav_activity_id = Column(
        Integer, ForeignKey("dav_activity.dav_activity_id"), nullable=False, unique=True
    )
