...
class ArchiveActivity(Base):
    ...
    def __eq__(self, other):
        ...
        if not isinstance(other, ArchiveActivity):
            return False
        return (
            self.archive_activity_id is not None
            and self.archive_activity_id == other.archive_activity_id
        )

    def __hash__(self):
        """Hash based on archive_activity_id for set/dict operations."""
        return (
            hash(self.archive_activity_id)
            if self.archive_activity_id is not None
            else hash(id(self))
        )
...
class ArchiveCharge(Base):
    ...
    archive_activity_id = Column(
        Integer,
        ForeignKey("archive_activity.archive_activity_id"),
        nullable=False,
        unique=True,
    )
