...
def generate_charge_breakdown_bars(charge_totals: Dict[str, float], width: int = 400, height: int = 200) -> str:
    """
    Generate a simple SVG bar chart showing charge breakdown by type.

    Args:
        charge_totals: Dict with keys: comp, dav, disk, archive, total
        width: Chart width in pixels
        height: Chart height in pixels
    ...
    for key in ['comp', 'dav', 'disk', 'archive']:
        value = charge_totals.get(key, 0)
        if value > 0:
            charge_types.append((key.upper(), value))

    if not charge_types:
        return '<div class="text-muted">No charges recorded.</div>'
    ...
    num_bars = len(charge_types)
    bar_spacing = 10
    bar_width = (chart_width - (num_bars - 1) * bar_spacing) / num_bars

    max_value = max(val for _, val in charge_types)
    ...
    bars = ""
    labels = ""
    for i, (charge_type, value) in enumerate(charge_types):
        x = padding + i * (bar_width + bar_spacing)
        bar_height = (value / max_value) * chart_height
        y = padding + chart_height - bar_height

        color = colors.get(charge_type, '#6c757d')

        bars += f'<rect x="{x:.2f}" y="{y:.2f}" width="{bar_width:.2f}" height="{bar_height:.2f}" fill="{color}"/>\n'

        # Value label on top of bar
        labels += f'<text x="{x + bar_width/2:.2f}" y="{y - 5:.2f}" text-anchor="middle" font-size="12" fill="#333">{format_number(value)}</text>\n'

        # Type label below bar
        labels += f'<text x="{x + bar_width/2:.2f}" y="{padding + chart_height + 20}" text-anchor="middle" font-size="12" fill="#666">{charge_type}</text>\n'
    ...
    <svg width="{width}" height="{height}" xmlns="http://www.w3.org/2000/svg">
        ...
        <!-- Chart title -->
        <text x="{width/2}" y="20" text-anchor="middle" font-size="14" font-weight="bold" fill="#333">Charges by Type</text>
    </svg>
    """
