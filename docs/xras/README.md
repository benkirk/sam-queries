...
| [`XRAS_WRITE_FIXUPS.md`](outgoing/XRAS_WRITE_FIXUPS.md) | ⚠️ **Research, not built.** The **write** half § 4.7 left closed: what the credential can mutate (merge ✅, unreconcile ❌, withdraw ✅ role-scoped, delete ❌), proven live, and the design for a `MANAGE_XRAS` control that clears an erroneously-reconciled placeholder off *Accounts Needed* by merging it into the right real identity. **Addendum**: the sibling capability — withdrawing an action to close a stale **Approved** request (→ Incomplete, drops off Pending Requests) |
