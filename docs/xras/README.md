...
| [`XRAS_SUBMISSION.md`](outgoing/XRAS_SUBMISSION.md) | **Design, not built.** Authoring requests from SAM to close the loop (SAM → XRAS review → SAM apply): why resolving the PI, mnemonic, facility and contract *at submission* retires most of the `New` 422 catalog; `requestId` is the key that survives the `NCAR####` → projcode rewrite; `opportunityQA` is a measured round-trip channel SAM currently discards; phase 1 is existing-project actions |
