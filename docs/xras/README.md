...
| [`XRAS_SUBMISSION.md`](../plans/XRAS_SUBMISSION.md) | **Design, not built; the authoring surface is measured** (record: [`outgoing/XRAS_SUBMISSION_PROBES.md`](outgoing/XRAS_SUBMISSION_PROBES.md)). Authoring requests from SAM to close the loop (SAM → XRAS review → SAM apply) in three phases: Supplement/Extension on existing projects from the lead's dashboard, routed by the XRAS-family predicate and an allocation-type policy; new requests by logged-in users bounded by per-type, per-resource limits; requests by people without an account, via [`ACCOUNT_REGISTRATION.md`](../plans/ACCOUNT_REGISTRATION.md). One flow layer serves the PI form and the admin editor |
