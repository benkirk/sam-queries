...
| R1b | `GET /v1/projects` (submit, `benkirk`) | **404 with SAM's own envelope** — `no route for GET /api/xras/v1/users/projects/benkirk`. XRAS proxies this route to the accounting service, i.e. back to us (`XRAS_WRITE_PROBES.md` § 4.6). Not an origin oracle; a SAM route we do not serve |
