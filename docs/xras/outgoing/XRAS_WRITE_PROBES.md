...
| 6 | Label every validate verdict with the user it was evaluated as; never cache across users; treat a failed preflight as non-terminal | P2 — the same action validates as the PI and fails as the Allocation Manager | The caveat, the labeling, and a chunk of the re-submit modal's copy |
