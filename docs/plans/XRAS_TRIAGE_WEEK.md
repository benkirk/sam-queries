...
`docs/plans/implemented/XRAS_DATA_MODEL_UPLIFT.md` (what PR #479 changed and why). The
...
| 15:47 (25th) | `sha-11a7729` rolled out: the identity-merge feature (`docs/plans/implemented/XRAS_IDENTITY_MERGE.md`). Healthcheck 41/3/0. First render already found work the morning's probe could not: `ggeogdzhayev-user-7016v` reads **Ready to merge → `geogdzhayev`** (NCAR4261) — the SAM account was created at 14:37Z, after the sweep, and the row flipped at render with no sweep. `--identity-report`: 1 target, 6 need an account. |
| 16:58 (25th) | `sha-acd3d95` rolled out: Track B of `implemented/XRAS_DATA_MODEL_UPLIFT.md` (allocationDateType, the family seam, the affiliation message, identity badge, add-role copy). Healthcheck 42/2/0. A live re-check of NCAR4262 now reads **PI `glarouche` has no current institution or organization in SAM** — the affiliation class named as itself. |
...
4. "Recent submissions" drops a row once it is posted **and notified**, and
   ...
   their queue and in ours (`docs/plans/implemented/XRAS_PENDING_WORK.md`).
