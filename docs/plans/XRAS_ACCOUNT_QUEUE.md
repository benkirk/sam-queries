...
in [`ACCOUNT_REGISTRATION.md`](implemented/ACCOUNT_REGISTRATION.md). Everything here was scoped during the
...
| ✅ ~~**Unit tests reach the live XRAS API**~~ **FIXED 2026-08-21** | The suite inherited a real, **write-provisioned** `XRAS_API_KEY` from `.env` (`load_dotenv` runs at import), and `xras_api_configured()` measured **True** inside it. Harmless while the client was GET-only; not harmless once a write client existed, since person merge deletes an account with no undo. Now guarded twice, mirroring the SMTP precedent: `pytest_configure` occupies `XRAS_API_KEY` / both levers **before** dotenv fills them, and an autouse fixture blocks any real outbound HTTP at `requests.Session.request`. Both are asserted in `tests/unit/test_outbound_guards.py`, including that the established instance-level mocking idiom still works through the guard |
