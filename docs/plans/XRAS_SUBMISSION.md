...
| **3 — a request by someone without an account** | `ACCOUNT_REGISTRATION.md` first. Then: the submission is created under an XRAS person SAM mints (`POST /v1/people/<u>` with `isReconciled = false` — a new verb, to be probed), the `xras_submission` row carries `registration_id`, and the account's arrival merges the placeholder into the real username with `merge_placeholder`, re-pointing the PI role | 2 in production; the registration queue |
